...
interface DayStatus {
  ...
  perHeadCost: { breakfast: number; lunch: number; dinner: number };
}
...
interface BillData {
  ...
  mealStatusByDay: Record<string, DayStatus>;
}
...
interface StudentBillModalProps {
  open: boolean;
...
function autoColWidths(data: Record<string, unknown>[], headers: string[]) {
...
const MealPill = ({ active, label, icon: Icon, guest }: MealPillProps) => (
  <span
    className={cn(
...
const StatCard = ({ label, icon: Icon, bgClass, textClass, count, cost }: StatCardProps) => (
...
function exportToExcel({ data, selectedDate, wing }: ExportParams) {
  const { studentDetails: s, totalMonthlyCost, mealStatusByDay } = data;
  const month = format(selectedDate, "MM");
  const year  = format(selectedDate, "yyyy");
  const monthLabel = format(selectedDate, "MMMM yyyy");

  const rows = Object.entries(mealStatusByDay).map(([date, st]: [string, DayStatus]) => {
    const c = dayCost(st);
    return {
      "Date":               date,
...
export const StudentBillModal = ({ open, onOpenChange, data, selectedDate, wing }: StudentBillModalProps) => {
  const { studentDetails: s, totalMonthlyCost, mealStatusByDay } = data;
        ...
        */}
        <div className="flex-1 min-h-0 overflow-y-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {days.map(([dateStr, status]: [string, DayStatus]) => {
