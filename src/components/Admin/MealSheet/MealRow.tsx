...
interface MealRowProps {
  student: StudentMealData;
  breakfastFeast: boolean;
  lunchFeast: boolean;
  dinnerFeast: boolean;
  date: string;
...
const MealBtn = memo(({ active, feast, onClick, label }: MealBtnProps) => (
  <button
    onClick={onClick}
    className={cn(
...
const GuestStepper = memo(({ label, value, onChange }: GuestStepperProps) => (
...
export const MealRow = memo(
  ({
    ...
    const setMeal = useCallback((meal: string) => (value: number) => {
    ...
    const handleToggle = useCallback(
      async (meal: string) => {
        try {
          ...
          const msg = e?.response?.data?.message || `Failed to toggle ${meal}`;
          toast.error(msg);
        }
      },
