 ...
 *
 * Props
 *   page             – current 1-based page (required)
 *   totalPages       – total number of pages (required)
 *   total            – total item count shown as "X–Y of Z"
 *   pageSize         – items per page (used for X–Y calculation)
 *   onPageChange     – (newPage: number) => void
 *   onPageSizeChange – (newSize: number) => void  (optional — shows the per-page selector when provided)
 */
export default function Pagination({ page, totalPages, total, pageSize, onPageChange, onPageSizeChange }) {
  ...
  const navBtn = (enabled) =>
    cn(
      ...
      enabled ? "hover:bg-muted/60 hover:border-border cursor-pointer" : "opacity-40 cursor-not-allowed"
    );
