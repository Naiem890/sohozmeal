...
async function updateBillStudentsCount(
  ...
  try {
    ...
    if (includeAllStudents) {
      ...
      const totalStudentsWithMeal =
        studentsWithMealOn.length > 0 ? studentsWithMealOn[0].count : 0;
    ...
    console.error(
      `Error updating student count in the bill for ${meal} on ${dateWithoutTime}:`,
      error,
    );
  }
}
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const hallFeast = await HallFeast.findOne({
        date,
        meal,
      ...
      if (hallFeast) {
        return res
          ...
          .json({
            status: "on",
            message: `${meal} is on for ${wing} wing on ${date}`,
          });
      } else {
        return res
          .status(200)
          .json({
            status: "off",
            message: `${meal} is off for ${wing} wing on ${date}`,
          });
      }
    } catch (error: any) {
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      const hallFeast = await HallFeast.findByIdAndUpdate(
        req.params.id,
        req.body,
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const hallFeast = await HallFeast.find({
        date,
        wing: wing.toUpperCase(),
      });
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!deletedHallFeast) {
        return res
          .status(404)
          .json({ error: "No HallFeast found for this date, meal, and wing" });
      }
      await updateBillStudentsCount(date, meal, wing.toUpperCase(), false);
      res
        .status(200)
        .json({ message: "HallFeast deleted successfully", deletedHallFeast });
    } catch (error: any) {
      res.status(500).json({ error: "Internal server error" });
    }
  },
