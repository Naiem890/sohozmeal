...
function calculatePerHeadCost(mealBill: { totalCost: number; totalStudent: number }): number {
  return mealBill.totalStudent > 0 ? r4(mealBill.totalCost / mealBill.totalStudent) : 0;
}

router.post('/generate-bills', validateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const result = students.map((student) => ({
      ...student.toObject(),
      totalCost: studentInfo[student.studentId] ? r2(studentInfo[student.studentId].totalCost) : 0,
    }));
...
router.post('/sync', validateToken, async (req: Request, res: Response) => {
  try {
    ...
    await Promise.all(Array.from(allAffectedDates).map((dateStr) => createOrUpdateBill(dateStr, outWing)));
...
router.post('/', validateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'Bill generated successfully', date: dateObj, wing: wing.toUpperCase(), mealBill: bill.mealBill });
  } catch (error) {
    console.error('Error during bill generation:', error);
    res.status(500).json({ message: 'An error occurred while generating the bill' });
  }
});
...
router.get('/student', validateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const billsPipeline: any[] = [
      { $match: { date: { $gte: startDate, $lt: endDate }, wing: wing.toUpperCase() } },
      {
        $addFields: {
          'mealBill.breakfast.perHeadCost': { $cond: [{ $ne: ['$mealBill.breakfast.totalStudent', 0] }, { $round: [{ $divide: ['$mealBill.breakfast.totalCost', '$mealBill.breakfast.totalStudent'] }, 4] }, 0] },
          'mealBill.lunch.perHeadCost': { $cond: [{ $ne: ['$mealBill.lunch.totalStudent', 0] }, { $round: [{ $divide: ['$mealBill.lunch.totalCost', '$mealBill.lunch.totalStudent'] }, 4] }, 0] },
          'mealBill.dinner.perHeadCost': { $cond: [{ $ne: ['$mealBill.dinner.totalStudent', 0] }, { $round: [{ $divide: ['$mealBill.dinner.totalCost', '$mealBill.dinner.totalStudent'] }, 4] }, 0] },
        },
      },
      { $project: { date: { $dateToString: { format: '%Y-%m-%d', date: '$date' } }, mealBill: 1, wing: 1 } },
      { $sort: { date: 1 } },
    ];
    ...
    res.status(200).json({ message: 'Bills and meals fetched successfully', mealBillData: combinedMealBill });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'An error occurred while fetching bills and meals' });
  }
});
...
router.post('/monthly', validateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: `Bills generated successfully for the ${wing} wing for the month of ${month}-${year}`, result });
  } catch (error) {
...
router.get('/monthly/all', validateToken, async (req: Request, res: Response) => {
  try {
    ...
    const students = await Student.find(studentFilter, { studentId: 1, name: 1, hallId: 1, batch: 1, department: 1, roomNo: 1, gender: 1, residence: 1, _id: 0 })
      .sort({ hallId: 1 })
    ...
    const studentDetailsById = students.reduce((acc: any, student) => {
      acc[student.studentId] = { name: student.name, hallId: student.hallId, batch: student.batch, department: student.department, roomNo: student.roomNo, gender: student.gender, residence: student.residence };
      return acc;
    }, {});
    ...
    const [meals, hallFeasts, costs] = await Promise.all([
      Meal.find({ studentId: { $in: studentIds }, date: { $gte: startDateString, $lte: endDateString } }, { studentId: 1, date: 1, meal: 1, guestMeal: 1, _id: 0 }).exec(),
      HallFeast.find({ date: { $gte: new Date(startDateString), $lte: new Date(endDateString) }, wing: wing.toUpperCase() }).exec(),
      Cost.find({ date: { $gte: new Date(startDateString), $lte: new Date(endDateString) }, wing: wing.toUpperCase() }).exec(),
    ]);
    ...
    studentIds.forEach((studentId) => {
      ...
      for (let day = 1; day <= new Date(year, month, 0).getDate(); day++) {
        ...
        mealStatusByStudent[studentId][formattedDate] = { breakfast: false, lunch: false, dinner: false, perHeadCost: { breakfast: 0, lunch: 0, dinner: 0 }, guestMeal: { breakfast: 0, lunch: 0, dinner: 0 } };
      }
    });

    const calcPerHeadCost = (totalCost: number, totalStudent: number) => (totalStudent > 0 ? r4(totalCost / totalStudent) : 0);
    ...
    meals.forEach((meal) => {
      ...
      if (mealStatusByStudent[meal.studentId]) {
        mealStatusByStudent[meal.studentId][formattedDate] = {
          ...
          guestMeal: { breakfast: meal?.guestMeal?.breakfast || 0, lunch: meal?.guestMeal?.lunch || 0, dinner: meal?.guestMeal?.dinner || 0 },
        };
      }
    });
    ...
    hallFeasts.forEach((feast) => {
      ...
      studentIds.forEach((studentId) => {
        if (mealStatusByStudent[studentId]?.[formattedDate]) {
          (mealStatusByStudent[studentId][formattedDate] as any)[feast.meal] = true;
        }
      });
    });
    ...
    costs.forEach((cost) => {
      ...
      const perHeadBreakfastCost = calcPerHeadCost(cost.mealBill.breakfast.totalCost, cost.mealBill.breakfast.totalStudent);
      const perHeadLunchCost = calcPerHeadCost(cost.mealBill.lunch.totalCost, cost.mealBill.lunch.totalStudent);
      const perHeadDinnerCost = calcPerHeadCost(cost.mealBill.dinner.totalCost, cost.mealBill.dinner.totalStudent);
    ...
    Object.keys(studentMonthlyCosts).forEach((id) => { studentMonthlyCosts[id] = r2(studentMonthlyCosts[id]); });
    ...
    res.status(200).json({
      ...
      studentMonthlyCosts,
      studentDetailsById,
      pagination: { page: pageNum, limit: limitNum, total: totalStudents, totalPages: Math.ceil(totalStudents / limitNum) },
    });
  } catch (error) {
...
router.get('/monthly/student', validateToken, async (req: Request, res: Response) => {
  try {
    const { month, year, studentId } = req.query as any;
    if (!month || !year || isNaN(month) || isNaN(year)) {
      return res.status(400).json({ error: 'Invalid month or year' });
    }
    if (!studentId) return res.status(400).json({ error: 'Invalid or missing studentId parameter' });

    const student = await Student.findOne({ studentId }, { studentId: 1, name: 1, hallId: 1, batch: 1, department: 1, roomNo: 1, gender: 1, residence: 1, _id: 0 }).exec();
    ...
    const [meals, hallFeasts, costs] = await Promise.all([
      Meal.find({ studentId, date: { $gte: startDateString, $lte: endDateString } }, { studentId: 1, date: 1, meal: 1, guestMeal: 1, _id: 0 }).exec(),
      HallFeast.find({ date: { $gte: new Date(startDateString), $lte: new Date(endDateString) }, wing: student.gender.toUpperCase() }).exec(),
      Cost.find({ date: { $gte: new Date(startDateString), $lte: new Date(endDateString) }, wing: student.gender.toUpperCase() }).exec(),
    ]);
    ...
    for (let day = 1; day <= new Date(year, month, 0).getDate(); day++) {
      const formattedDate = `${day.toString().padStart(2, '0')}-${month}-${year}`;
      mealStatusByDay[formattedDate] = { breakfast: false, lunch: false, dinner: false, perHeadCost: { breakfast: 0, lunch: 0, dinner: 0 }, guestMeal: { breakfast: 0, lunch: 0, dinner: 0 } };
    }
    ...
    Object.keys(mealStatusByDay).forEach((formattedDate) => {
      ...
      if (costForDay) {
        const perHeadCosts = {
          breakfast: calcPerHeadCost(costForDay.breakfast.totalCost, costForDay.breakfast.totalStudent),
          lunch: calcPerHeadCost(costForDay.lunch.totalCost, costForDay.lunch.totalStudent),
          dinner: calcPerHeadCost(costForDay.dinner.totalCost, costForDay.dinner.totalStudent),
        };
