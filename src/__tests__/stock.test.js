...
async function seedOUT(itemId, qty, unitPrice, date = DATE, meal = "LUNCH") {
  const tx = await StockTransaction.create({
    item: itemId,
    quantityChange: qty,
    type: "OUT",
...
describe("StockItem CRUD", () => {
  ...
  describe("POST /stock/item", () => {
    ...
    it("returns 500 for invalid unit enum value", async () => {
      const res = await request(app)
        .post("/stock/item")
  ...
  describe("GET /stock/item", () => {
    ...
    it("filters items by wing=MALE", async () => {
      ...
      expect(res.body.stockItems.every((i) => i.wing === "MALE")).toBe(true);
    });
    ...
    it("sorts by category descending (STORED before NON_STORED)", async () => {
      const res = await request(app).get("/stock/item?wing=MALE");
      const cats = res.body.stockItems.map((i) => i.category);
...
describe("Stock OUT", () => {
  ...
  describe("STORED OUT", () => {
    ...
    it("returns 400 for invalid wing", async () => {
      ...
      expect(res.status).toBe(400);
      expect(res.body.error).toMatch(/invalid.*wing/i);
    });
...
describe("Transaction GET", () => {
  ...
  describe("GET /stock/transactions/all", () => {
    ...
    it("filters by wing", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.every((t) => t.wing === "MALE")).toBe(true);
      expect(res.body).toHaveLength(3);
    });
  ...
  describe("GET /stock/transactions", () => {
    ...
    it("filters by type=IN", async () => {
      ...
      expect(res.status).toBe(200);
      expect(res.body.transactions.every((t) => t.type === "IN")).toBe(true);
    });
