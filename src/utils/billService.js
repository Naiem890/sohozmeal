...
async function createOrUpdateBill(date, wing) {
  try {
    ...
    // Resolve students for this wing using the Student schema field.
    ...
    if (breakfastFeastExists) {
      ...
      const breakfastMealCount = await Meal.countDocuments({
        "meal.breakfast": true,
        date: formattedDate,
    ...
    if (lunchFeastExists) {
      ...
      const lunchMealCount = await Meal.countDocuments({
        "meal.lunch": true,
        date: formattedDate,
        studentId: { $in: studentIds },
      });
    ...
    if (dinnerFeastExists) {
      ...
      const dinnerMealCount = await Meal.countDocuments({
        "meal.dinner": true,
        date: formattedDate,
        studentId: { $in: studentIds },
      });
